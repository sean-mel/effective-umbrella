...
double deg2rad(uint32_t n) {
    const double x = 0.001;  // store constants during compilation to reduce runtime
    const double k = x * M_PI / 180;
    double y = 1;
...
double grav(uint32_t n) {
    ...
    const double r = 1.5e12;          // distance apart
    const double r2 = r * r;
    const double a = G * m2 / r2;
    const double F = a * m1;

    constexpr double dt = 0.1;       // 0.1 second timestep
    for (uint32_t i = 0; i < n; i++) {
        x = x +  (v + 0.5 * a * dt) * dt; // rearranged equations for less computations, took out dt
        v = v + a * dt;
    }
    return x;
}
 ...
 */
void f(); // benchmark for function call
...
int32_t countPrimes2(uint32_t n) {
	uint32_t count = 1; // special case for 2, the only even prime
  	for (uint32_t i = 3; i <= n; i += 2) {
  		for (uint32_t j = 3; j < sqrt(i); j++ ) // divide by sqrt i
			if (i % j == 0)
				goto notPrime;
		count++;
	notPrime: ; // do nothing if not prime
...
void matmult4(float ans[], float a[], float b[], uint32_t n) {
	for (uint32_t i = 0, c = 0; i < n; i++) 
		for (uint32_t j = 0, e = n*c; j < n; j++, c++) {
			double dot = 0;
			for (uint32_t k = 0, d = n*i; k < n; k++)
				dot += a[d] * b[e];
			ans[c] = dot;
		}
}
...
template<typename Func> // f call benchmark
void benchmarkf(const char msg[], Func f, uint32_t n) {
	for (uint32_t trials = 0; trials < 10; trials++) {
		clock_t t0 = clock();
		f();
		clock_t t1 = clock();
		cout << setw(12) << msg << setw(18) <<'\t' << (t1-t0) << '\n';
	}
}
int main() {
